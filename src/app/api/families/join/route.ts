...
export async function POST(req: import("next/server").NextRequest) {
  ...
  if (!inviteCode) return NextResponse.json({ error: "Codigo obrigatorio" }, { status: 400 });
  ...
  if (!invite) return NextResponse.json({ error: "Codigo invalido ou expirado" }, { status: 400 });
  ...
  return NextResponse.json({ message: "Voce entrou na familia com sucesso", familyId: invite.familyId });
}
