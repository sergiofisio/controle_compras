...
export async function POST(req: import("next/server").NextRequest) {
  ...
  if (listId) {
    ...
    if (!list || list.familyId !== familyId) {
      return NextResponse.json({ error: "Lista nao encontrada para a familia ativa" }, { status: 404 });
    }
  } else {
    if (!listName) return NextResponse.json({ error: "listName obrigatorio para criar lista" }, { status: 400 });
    const normalizedListName = String(listName).trim();
    if (!normalizedListName) return NextResponse.json({ error: "Nome da lista invalido" }, { status: 400 });
    ...
    return NextResponse.json({ error: "Crie a lista primeiro e depois adicione itens nela" }, { status: 400 });
  }

  if (!itemName) return NextResponse.json({ error: "itemName obrigatorio para adicionar item" }, { status: 400 });
...
export async function DELETE(req: import("next/server").NextRequest) {
  ...
  if (!listId && !itemId) return NextResponse.json({ error: "listId ou itemId obrigatorio" }, { status: 400 });
  ...
  if (itemId) {
    ...
    if (!itemList) return NextResponse.json({ error: "Lista do item nao encontrada" }, { status: 404 });
...
export async function PATCH(req: import("next/server").NextRequest) {
  ...
  if (!itemId || typeof checked !== "boolean") {
    return NextResponse.json({ error: "itemId e checked obrigatorios" }, { status: 400 });
  }
