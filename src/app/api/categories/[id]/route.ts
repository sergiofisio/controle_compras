...
export async function DELETE(req: import("next/server").NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!category) return NextResponse.json({ error: apiT(req, "api.sectionNotFound") }, { status: 404 });
  if (category.isGlobal) return NextResponse.json({ error: apiT(req, "api.cannotRemoveGlobalSection") }, { status: 400 });
  if (category.familyId !== familyId) return NextResponse.json({ error: apiT(req, "api.sectionWrongFamily") }, { status: 403 });
  ...
  if (listUsage > 0 || purchaseUsage > 0 || brandUsage > 0) {
    return NextResponse.json({ error: apiT(req, "api.sectionInUse") }, { status: 409 });
  }
